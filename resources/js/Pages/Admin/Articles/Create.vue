...
const form = useForm({
    journal_id: props.journals?.[0]?.id || '',
    issue_id: '',
    title: '',
    slug: '',
    abstract: '',
    ...
    ethics_statement: 'Informed consent was obtained from all subjects involved in the study.',
    license: 'CC BY 4.0',
    status: 'published',
    authors: [
        {
...
<template>
    <Head title="Publish Article — Admin" />
    ...
    <AdminLayout>
        <template #header>
            <div class="flex items-center gap-3">
                ...
                <h1 class="text-xl font-bold text-gray-900">Publish New Scholarly Article</h1>
            </div>
        </template>
        ...
        <form @submit.prevent="submit" class="max-w-5xl space-y-8">
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-6 md:p-8 shadow-sm space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-gray-700 uppercase mb-1">Assign to Issue</label>
                        <select
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input
                            ...
                            type="text"
                            required
                            placeholder="Full scholarly title of manuscript..."
                            class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-navy-600 focus:outline-none"
                        />
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            <label class="block text-xs font-semibold text-gray-700 uppercase mb-1">DOI (e.g. 10.58421/kd.v1i1.01)</label>
                            <input
                                v-model="form.doi"
                                type="text"
                                placeholder="10.58421/..."
                    ...
                    >
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <div class="sm:col-span-2">
                                ...
                                <input v-model="author.affiliation" type="text" placeholder="e.g. Dept of Physics, Oxford University" class="w-full bg-white border border-gray-300 rounded px-2.5 py-1.5 text-sm" />
                            </div>
                            <div>
                        ...
                        <div class="flex items-center gap-2">
                            <input v-model="author.is_corresponding" type="checkbox" :id="`corr-${aIndex}`" class="rounded border-gray-300 text-navy-700 focus:ring-navy-700" />
                            <label :for="`corr-${aIndex}`" class="text-xs font-medium text-gray-700">Corresponding Author</label>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="space-y-3">
                    <div class="flex gap-2">
                        <textarea
                            ...
                            rows="2"
                            placeholder="Paste reference text (e.g. Smith, J. et al. (2024). Title of paper. Journal, 1(2), 34-45)..."
